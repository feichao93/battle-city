...
坦克大战当年红遍大江南北，很多和我一样的九零后在应该都有着对这个游戏的记忆。现在显示器分辨率越来越高，使用矢量图来实现像素风格游戏，可以获得非常高的展现质量。[该复刻版](http://shinima.pw/battle-city/) 是我花了很长时间折腾的坦克大战复刻版本，所有元素都使用矢量图（SVG）进行渲染，针对网页的交互方式重新设计了关卡编辑器，新增了关卡选择功能、自定义关卡管理功能等，另外该复刻版还包括了一个 Gallery 页面用于展示所有的游戏元素，想必该复刻版一定可以勾起你的儿时记忆。
...
[点击这里打开游戏](http://shinima.pw/battle-city/)。欢迎大家在 GitHub 上 star [本项目](https://github.com/shinima/battle-city)。本文后面的内容会大致介绍整个游戏的~~开发~~折腾过程，对 React / Redux 感兴趣的前端同学可以继续往下看。

------------------

复刻版主要包括了下面五个方面的内容：素材、数据、展现、逻辑、电脑玩家。
